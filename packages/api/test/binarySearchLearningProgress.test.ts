...
describe("Binary Search learning progress", () => {
  it("accepts the guide id and persists completion", async () => {
    const program = Effect.gen(function* () {
      const database = yield* DatabaseServiceTag;
      yield* database.migrate;
      const appUser = yield* Effect.promise(() => createTestAppUser(database, "binary_search_learner"));
      const caller = appRouter.createCaller({ database, judges, appUser });
      return yield* Effect.promise(async () => {
        await caller.learningProgress.start({ guideId: LEARNING_GUIDE_IDS.BinarySearch });
        await caller.learningProgress.setStatus({ guideId: LEARNING_GUIDE_IDS.BinarySearch, status: LEARNING_PROGRESS_STATUSES.Completed });
    ...
    expect(rows).toEqual([expect.objectContaining({ guideId: LEARNING_GUIDE_IDS.BinarySearch, status: LEARNING_PROGRESS_STATUSES.Completed })]);
  });
});
